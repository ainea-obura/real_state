...
const TAB_BG_OPACITY: Record<TabKey, string> = {
  all: "bg-gray-400/10",
  approved: "bg-green-500/10",
  pending: "bg-yellow-500/10",
  rejected: "bg-red-500/10",
  expired: "bg-gray-500/10",
};

const TenantVerification = ({ tenantId }: { tenantId: string }) => {
  ...
  const {
    ...
    queryKey: ["tenant-verification-documents", tenantId],
    queryFn: () => listVerificationDocuments(tenantId, "tenant"),
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["tenant-verification-documents", tenantId] });
      setModalOpen(false);
    },
    onError: (error) => {
  ...
  const statusMutation = useMutation({
    mutationFn: async ({ id, action }: { id: string; action: "approve" | "reject" | "expire" }) => {
      return updateVerificationStatus(id, action, "tenant");
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tenant-verification-documents", tenantId] });
    },
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        {needsDocument && (
          <PermissionGate codename="add_tenant_verification" showFallback={false}>
          <Button
            size="sm"
          ...
          {filteredDocs.map((doc) => (
            ...
            >
              ...
              <CardFooter className="flex gap-2 justify-end px-6 pt-0 pb-4">
                {doc.status === "approved" && (
                  <PermissionGate codename="edit_tenant_verification" showFallback={false}>
