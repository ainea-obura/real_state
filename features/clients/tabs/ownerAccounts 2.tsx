...
import { Plus, Edit, Trash2, CreditCard, Smartphone, Building2, AlertCircle } from "lucide-react";
...
const OwnerAccounts = ({ ownerId }: OwnerAccountsProps) => {
  ...
  const {
    ...
    queryKey: ["userAccounts", ownerId],
    queryFn: () => getUserAccounts(ownerId),
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => createAccount({ ...data, user_id: ownerId }),
    onSuccess: (response) => {
      if (response.error) {
        toast.error(response.message || "Failed to create account");
        return;
      }
      queryClient.invalidateQueries({ queryKey: ["userAccounts", ownerId] });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ accountId, data }: { accountId: string; data: any }) =>
      updateAccount(accountId, { ...data, user_id: ownerId }),
    onSuccess: (response) => {
      if (response.error) {
  ...
  const deleteMutation = useMutation({
    mutationFn: (accountId: string) => deleteAccount(accountId, ownerId),
    onSuccess: (response) => {
      if (response.error) {
  ...
  if (error) {
    return (
      <div className="flex flex-col justify-center items-center py-16">
        ...
        <h3 className="mb-2 text-lg font-semibold text-gray-900">Error loading accounts</h3>
        <p className="max-w-md text-center text-gray-600">
          {accountsResponse?.message || "Failed to load accounts. Please try again."}
        </p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Owner Accounts</h2>
          <p className="mt-1 text-gray-600">
            Manage payment accounts for receiving rent and other payments
          </p>
        </div>
