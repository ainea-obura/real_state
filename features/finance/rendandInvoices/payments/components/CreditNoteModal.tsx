...
interface CreditNoteModalProps {
  open: boolean;
  onClose: () => void;
  payment: any; // Payment data instead of invoice
}

const CreditNoteModal = ({ open, onClose, payment }: CreditNoteModalProps) => {
  const [creditData, setCreditData] = useState({
    type: "refund",
    amount: payment?.amountPaidNoCurrency || 0,
    reason: "",
    description: "",
    applyToFuture: false,
  ...
  const handleSubmit = () => {
    // Get invoice ID from the first invoice in invoicesApplied array
    const invoiceId = payment?.invoicesApplied?.[0]?.id;
    ...
    if (!creditData.reason || creditData.amount <= 0 || creditData.amount > payment?.amountPaidNoCurrency || !creditData.paymentType || !hasValidPaymentMethod || (requiresAccountNumber && !hasValidAccountNumber)) {
      toast.error("Please fill in all required fields correctly");
      return;
    }

    const mutationData = {
      invoice_id: invoiceId, // Use invoice ID from invoicesApplied
      amount: creditData.amount,
      reason: creditData.reason,
      description: creditData.description,
      paymentMethod: creditData.paymentMethod,
      ...
      <DialogContent className="min-w-3xl max-h-[calc(100vh-200px)] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex gap-2 items-center">
            ...
            Refund Payment - {payment?.invoicesApplied?.[0]?.invoiceNumber || payment?.paymentNumber}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-gray-50 rounded-lg">
            <h3 className="mb-2 font-semibold">Original Payment</h3>
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <div className="font-medium">{payment?.amountPaidNoCurrency}</div>
              </div>
              <div>
                <div className="text-gray-600">Invoice Number</div>
                <div className="font-medium">{payment?.invoicesApplied?.[0]?.invoiceNumber}</div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="font-medium">{payment?.property?.unit}</div>
              </div>
            </div>
          </div>
          ...
          <div className="p-3 bg-blue-50 rounded-lg">
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span>{payment?.amountPaid || 0}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between pt-2 font-semibold border-t">
                ...
                <span>{Math.max(0, (payment?.amountPaidNoCurrency || 0) - creditData.amount)}</span>
              </div>
            </div>
          </div>

          {/* Warning */}
          {creditData.amount > (payment?.amountPaidNoCurrency  || 0) * 0.5 && (
            <div className="p-2 bg-yellow-50 rounded-lg border border-yellow-200">
              ...
              <p className="mt-1 text-sm text-yellow-700">
                This credit represents more than 50% of the original payment
          ...
          <div className="flex gap-2 justify-end pt-3">
            ...
            <Button
              ...
              disabled={
                ...
                !creditData.reason ||
                creditData.amount <= 0 ||
                creditData.amount > payment?.amountPaidNoCurrency ||
            ...
            >
              ...
              {createCreditNoteMutation.isPending ? "Processing..." : "Refund Payment"}
            </Button>
          </div>
        </div>
      </DialogContent>
