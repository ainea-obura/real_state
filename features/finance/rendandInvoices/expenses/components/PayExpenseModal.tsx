...
import { Expense } from '@/features/finance/rendandInvoices/expenses/schema/expenseSchema';
...
const PayExpenseModal: React.FC<PayExpenseModalProps> = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="overflow-hidden p-0 mt-10 rounded-2xl shadow-2xl sm:max-w-2xl h-[calc(100vh-100px)]">
        {/* Gradient Header */}
        <ScrollArea className="max-h-[calc(100vh-100px)]">
        <div className="flex flex-col items-center px-6 py-5">
          <Coins className="mb-2 w-10 h-10 text-green-500" />
          <DialogTitle className="mb-1 text-2xl font-bold tracking-tight text-green-500">
            Pay Expense
          </DialogTitle>
          <div className="text-sm text-green-100">
            Please review and confirm this payment
          </div>
        </div>
        {/* Expense Summary */}
        <div className="px-6 pt-2 pb-4 space-y-3 bg-gray-50">
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-500">Expense Number</span>
            <span className="text-lg font-bold text-gray-900">{expense.expense_number}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-500">Vendor</span>
            <span className="font-semibold text-gray-900">{expense.vendor?.name || '-'}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-500">Total Amount</span>
            <span className="text-lg font-bold text-green-700">
              {expense.currency?.code || '$'} {expense.total_amount?.toLocaleString()}
            </span>
          </div>
          {expense.description && (
            <div className="flex justify-between items-center">
         ...
         {/* Tabs */}
         <Tabs value={tab} onValueChange={isPaying ? undefined : setTab} className="px-6 pt-2">
          <TabsList className="flex justify-between px-2 mb-4 w-full bg-transparent border-b-2 border-green-200">
            <TabsTrigger
              ...
              disabled={isPaying}
              tabIndex={isPaying ? -1 : 0}
              style={isPaying ? { pointerEvents: 'none', opacity: 0.5 } : undefined}
            >
              Mobile Money
            </TabsTrigger>
            <TabsTrigger
          ...
          <TabsContent value="paybill">
            <Form {...paybillForm}>
              <form className="pt-4 space-y-4">
                ...
                {paybillForm.watch('option') === 'PAYBILL' && (
                  <FormField
                    control={paybillForm.control}
                    name="paybill_number"
                    render={({ field }) => (
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{paybillForm.watch('option') === 'TILL' ? 'Till Number' : 'Account Number'}</FormLabel>
                      <FormControl>
                        <Input {...field} />
                      </FormControl>
                      <FormMessage />
        ...
        <DialogFooter className="flex gap-2 justify-end px-6 py-4 bg-gray-50 border-t">
          ...
          >
            {isPaying ? (
              <>
                <Loader2 className="mr-1 w-5 h-5 animate-spin" /> Paying...
              </>
            ) : (
              <>
                <Coins className="mr-1 w-5 h-5" /> Pay
              </>
            )}
          </Button>
        </DialogFooter>
