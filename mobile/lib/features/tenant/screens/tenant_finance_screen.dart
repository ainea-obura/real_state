...
import '../../../core/controllers/tenant_finance_controller.dart';
import '../../../core/models/invoice_model.dart';
import 'package:get/get.dart';
import 'invoice_detail_screen.dart';
import 'multi_invoice_payment_modal.dart';

class TenantFinanceScreen extends StatefulWidget {
  const TenantFinanceScreen({super.key});

  @override
  State<TenantFinanceScreen> createState() => _TenantFinanceScreenState();
}

class _TenantFinanceScreenState extends State<TenantFinanceScreen> {
  late TenantFinanceController _controller;
  final ScrollController _scrollController = ScrollController();

  @override
  void initState() {
    super.initState();
    _controller = Get.find<TenantFinanceController>();
    // Load data only when finance tab is accessed
  ...
  void didChangeDependencies() {
    ...
    // Only refresh if no data exists and not currently loading
    if (_controller.invoices.isEmpty && !_controller.isLoadingInvoices) {
      _controller.loadDataWhenTabAccessed();
    }
  }
          ...
          children: [
            ...
            Expanded(
              child: RefreshIndicator(
                ...
                child: GetBuilder<TenantFinanceController>(
                  builder: (controller) {
  ...
  Widget _buildStatusBadge(
    String status,
    bool isSelected, [
    TenantFinanceController? controller,
  ]) {
  ...
  Widget _buildDateRangeFilter([TenantFinanceController? controller]) {
    final ctrl = controller ?? _controller;
    final hasDateRange =
  ...
  void _showDateRangePicker([TenantFinanceController? controller]) async {
    final ctrl = controller ?? _controller;
  ...
  DateTimeRange? _getInitialDateRange([TenantFinanceController? controller]) {
    final ctrl = controller ?? _controller;
  ...
  Widget _buildInvoicesList(TenantFinanceController controller) {
  ...
  Widget _buildInvoiceCard(Invoice invoice) {
    ...
    final isSelected = _controller.selectedInvoiceIds.contains(invoice.id);
    final isSelectableForPayment = _controller.isInvoiceSelectableForPayment(
      invoice.id,
    );
      ...
      child: InkWell(
        onTap: () {
          if (_controller.isSelectionMode) {
            ...
            if (isSelectableForPayment) {
              _controller.toggleInvoiceSelection(invoice.id);
            }
          } else {
        ...
        onLongPress: () {
          if (!_controller.isSelectionMode) {
            ...
            _controller.selectInvoice(invoice.id);
          }
        },
        borderRadius: BorderRadius.circular(12),
        child: Padding(
  ...
  bool _hasActiveFilters() {
    return _controller.statusFilter.isNotEmpty ||
        _controller.dateFromFilter.isNotEmpty ||
        _controller.dateToFilter.isNotEmpty;
  }

  int _getActiveFilterCount() {
    int count = 0;
    count += _controller.statusFilter.length;
    if (_controller.dateFromFilter.isNotEmpty) count++;
    if (_controller.dateToFilter.isNotEmpty) count++;
    return count;
  }
  ...
  void _showMultiInvoicePaymentModal() {
    ...
    Get.dialog(
      MultiInvoicePaymentModal(
        ...
        totalAmount: _controller.totalSelectedAmount,
      ),
