...
class PaymentService {
  ...
  Future<Map<String, dynamic>> getPaymentStatus(String paymentId) async {
    try {
      final response = await _apiService.get(
        '/finance/payments/status/$paymentId/',
        includeAuth: true,
      );
      ...
      return {
        'success': false,
        'message': 'Failed to get payment status: ${e.error.message}',
      };
    } catch (e) {
      return {
        'success': false,
  ...
  Future<Map<String, dynamic>> cancelPayment(String paymentId) async {
    try {
      final response = await _apiService.post(
        '/finance/payments/cancel/$paymentId/',
        body: {},
        includeAuth: true,
      );
      ...
      return {
        'success': false,
        'message': 'Failed to cancel payment: ${e.error.message}',
      };
    } catch (e) {
      return {
        'success': false,
  ...
  Future<Map<String, dynamic>> getPaymentReceipt(String paymentId) async {
    try {
      final response = await _apiService.get(
        '/finance/payments/receipt/$paymentId/',
        includeAuth: true,
      );
      ...
      return {
        'success': false,
        'message': 'Failed to get payment receipt: ${e.error.message}',
      };
    } catch (e) {
      return {
        'success': false,
        'message': 'Failed to get payment receipt: ${e.toString()}',
      };
    }
  }
